...
def euler_modificado(expressao, h, a, b, y0):
    resultados = []
    ...
    while xi < b:
        yi_2 = (yi + expressao.subs(x, xi).subs(y, yi)*h/2)
        f = expressao.subs(x, xi + h/2).subs(y, yi_2)
        yi = (yi + f*h).evalf()
        xi += h
        resultados.append((xi, yi))
...
def main():
    ...
    resultados = euler_modificado(expressao, amplitude, limite_inf, limite_sup, valor_inicial)
    
    # escreve os resultados no arquivo de saida
    common.escrever_arquivo(arquivo_saida, f"Resultado do metodo de Euler Modificado\n[ ")
